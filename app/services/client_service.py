...
"""
        ...
        db = get_db()
        cursor = db.execute('''
            INSERT INTO clients (name, config_content, local_port, remote_port, server_addr, server_port, token, user, enabled, always_on, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?, 'stopped')
        ''', (name, config_content, local_port, remote_port, server_addr, server_port, token, user, 1 if always_on else 0))
        db.commit()
    ...
    def update_client(client_id: int, data: Dict) -> Tuple[bool, Dict]:
        ...
        always_on = data.get('always_on', client.get('always_on', False))
        local_port = data.get('local_port', client.get('local_port', 0))
        remote_port = data.get('remote_port', client.get('remote_port', 0))
        server_addr = data.get('server_addr', client.get('server_addr', ''))
        server_port = data.get('server_port', client.get('server_port', 7000))
        token = data.get('token', client.get('token', ''))
        user = data.get('user', client.get('user', ''))
        ...
        # 如果 always_on 客户端被禁用，发送告警
        if not enabled and client.get('always_on') and client.get('enabled'):
            AlertService.send_alert(
                client_name=name,
                alert_type='always_on_failed',
                message=f'Always-On 客户端 {name} 已被禁用'
            )

        db.execute('''
            UPDATE clients SET name = ?, enabled = ?, always_on = ?, local_port = ?, remote_port = ?, server_addr = ?, server_port = ?, token = ?, user = ?, updated_at = CURRENT_TIMESTAMP
            WHERE id = ?
        ''', (name, 1 if enabled else 0, 1 if always_on else 0, local_port, remote_port, server_addr, server_port, token, user, client_id))
        db.commit()
